...
export function BreadcrumbsRoot({ children, className = "", ariaLabel }) {
  const { t } = useT();
  return (
    <nav
...
export function Breadcrumb({ href, onClick, children }) {
  const handle = (e) => {
    if (onClick) {
      e.preventDefault();
      onClick(e);
...
export function BreadcrumbCurrent({ children }) {
  return (
