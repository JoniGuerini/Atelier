...
import { PageHead, Section, ThemeToggle } from "../ds/primitives.tsx";
...
function CopyableValue({ children, value, label }: any) {
  ...
  const style = { "--copied-label": `"✓ ${t("common.copied").toUpperCase()}"` } as CSSProperties;
  return (
    <button
      type="button"
...
function Swatch({ name, token, light, dark, resolved }: any) {
  const { t } = useT();
