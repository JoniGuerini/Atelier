      ...
      tasks: [
        {
          ...
          scope: [
            "Auditoria (Abril 2026): zero ficheiros .jsx/.js em src/ — stack já é TS.",
            "Corrigir `npx tsc --noEmit` (Motion JSX, recipes Step, seo LocaleId, Changelog ?raw, Code bash, tr()+ReactNode, polyfill takeRecords).",
            "Próximo passo opcional: strictNullChecks: true em tsconfig.json (refactor alargado).",
          ],
        },
        {
          id: "8.3",
          ...
          scope: [
            "Entregue (Abril 2026): stack em #ds-toaster-portal em document.body.",
            "createRoot só na primeira notificação; fila vazia → unmount + remove do DOM.",
            "Context + timers permanecem no <Toaster> da árvore principal; só a UI dos toasts vive no segundo root.",
          ],
        },
        {
          id: "8.4",
          ...
          scope: [
            "Entregue (Abril 2026): public/icons.svg com <symbol> + componente SpriteIcon.",
            "Migrados primeiro os microns de UI de alto uso: SidebarToggle, NavMode, BackToTop, ThemeToggle (primitives) + GitHub/LinkedIn (Footer).",
            "Chart, CircularProgress, avatars decorativos e SVGs dinâmicos permanecem inline — trade-off documentado no código.",
          ],
        },
        {
          id: "8.5",
          ...
          scope: [
            "Entregue (Abril 2026): @playwright/test + playwright.config.ts (webServer Vite :5174, Chromium).",
            "e2e/critical-flows.spec.ts — 5 fluxos: tema + reload, palette (trigger Search) → drag-drop, Drawer+Esc, combobox creatable+chips, kanban pointer drag.",
            "Scripts npm: e2e, e2e:ui; artefactos ignorados em .gitignore.",
          ],
        },
        {
          id: "8.6",
